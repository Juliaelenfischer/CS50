...
def buy():
    ...
    if request.method == "GET":
        ...
        # UPDATE table_name SET column1 = value 1... WHERE...
        db.execute("UPDATE users SET cash = ? WHERE id = ?",
                   updt_cash, user_id)  # Corregir id a user_id
        ...
        # Redirect to portfolio page after successful purchase
        return redirect("/")
...
def quote():
    ...
    if request.method == "GET":
        ...
        # Render template with stock information
...
def sell():
    ...
    if request.method == "GET":
        ...
        user_shares = db.execute(
            "SELECT SUM(shares) AS total_shares FROM transactions WHERE user_id = :user_id AND symbol = :symbol", user_id=user_id, symbol=symbol)
        ...
        db.execute("UPDATE users SET cash = cash + :transaction_value WHERE id = :user_id",
                   transaction_value=transaction_value, user_id=user_id)

        date = datetime.datetime.now()

        db.execute("INSERT INTO transactions (user_id, symbol, shares, price, date) VALUES (:user_id, :symbol, :shares, :price, :date)",
                   user_id=user_id, symbol=stock["symbol"], shares=(-1) * shares, price=stock["price"], date=date)
